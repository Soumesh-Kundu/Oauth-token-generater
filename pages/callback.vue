<template>
    <div class="flex items-center justify-center h-screen px-4 bg-slate-100">
        <div v-if="(scope!==undefined && code!==undefined) && isLoading" class="flex flex-col items-center w-full max-w-xs gap-5 px-5 py-8 bg-white rounded-md">
            <div role="status">
...
const scope = useRoute().query.scope
const downloadRef = ref(null)
const isLoading = ref(true)
onMounted(async () => {
    if(scope===undefined || code===undefined){
        navigateTo('/')
    }
...
function generateDownloadLink(data) {
    ...
    const bufferData = new Buffer.from(strData)
    const fileData = new Uint8Array(bufferData)
